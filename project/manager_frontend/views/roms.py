...
class RomListView(MultiFormView):
    ...
    def init_system(self):
        ...
        self.system_path = os.path.join(settings.RHGAMESTATION_ROMS_PATH, self.system_key)
        ...
        default_manifest = settings.RHGAMESTATION_SYSTEM_DEFAULT
        default_manifest.update({
            'key': self.system_key,
            'name': self.system_key
        })
        # Get the system manifest part if any, else a default dict
        self.system_manifest = RHGAMESTATION_MANIFEST.get(self.system_key, default_manifest)
    ...
    def check_system_bios(self):
        ...
        if self.system_manifest.get('bios', False):
            ...
            for item in bios_filenames:
                if not os.path.exists(os.path.join(settings.RHGAMESTATION_BIOS_PATH, item)):
                    missing.append(item)
            return missing
