...
class SystemsListView(FormView):
    ...
    def get_system_list(self):
        path = settings.RHGAMESTATION_ROMS_PATH
        existing_sys = []
        available_sys = []
        ...
        for item in os.listdir(path):
            ...
            if os.path.isdir(os.path.join(path, item)) and not item.startswith('.'):
                ...
                if item in RHGAMESTATION_MANIFEST:
                    existing_sys.append( (item, RHGAMESTATION_MANIFEST[item]['name']) )
                # Unknowed dirname
                else:
                    existing_sys.append( (item, item) )
        
        # Get system available: the ones that dont allready have a directory in systems dir
        for sys_key, sys_values in RHGAMESTATION_MANIFEST.items():
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'systems_path': settings.RHGAMESTATION_ROMS_PATH,
