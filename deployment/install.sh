...
function usage()
{
    echo "'rhgamestation-manager' installer"
    echo ""
    echo "./install.sh"
    echo "    -h --help"
    echo ""
    ...
    echo ""
    echo "    --compatible"
    echo "      Optionnal, use requirements compatible with RHGamestation 3.2.11"
    echo ""
}
...
wget https://github.com/sveetch/rhgamestation-manager/archive/$RELEASE.zip
unzip $RELEASE.zip
echo

echo "* Initialize virtual environment for project"
cd rhgamestation-manager-$RELEASE/
...
echo
echo "DONE!"
echo "Go into directory 'rhgamestation-manager-$RELEASE/' and you can start the server with the following command:"
echo
